...
const URGENT_PATTERNS = [
  /order\.?appointment/i,
  /appointment\.?(pending|urgent|validation)/i,
  /rendez[\s-]?vous\s+(urgent|validation)/i, // French
  /payment\.?failed/i,
...
const ACTION_PATTERNS = [
  ...
  // Appointments (non-urgent)
  /new\s+appointment/i,
  /nouveau\s+rendez[\s-]?vous/i, // French
  /appointment\s+(booked|confirmed|created)/i,
  /rendez[\s-]?vous\s+(réservé|confirmé|créé)/i, // French
  // Support
