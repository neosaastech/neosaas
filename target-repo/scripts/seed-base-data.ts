...
async function main() {
  ...
  // ── 4. VAT rates ────────────────────────────────────────────────────────────
  // NOTE: vat_rates has no unique constraint on (name, country) — a bare
  // "ON CONFLICT DO NOTHING" here never actually conflicts on anything (the
  // id is a fresh UUID every time), so this used to insert 4 new rows on
  // every single build. Confirmed in production: ~20 duplicate batches
  // accumulated between Feb and Jul 2026. Guard with an explicit existence
  // check instead of relying on a constraint that doesn't exist.
  console.log('\n📍 Seeding VAT rates...');
  await sql`
    INSERT INTO vat_rates (name, country, rate, description, is_default, is_active)
    SELECT * FROM (VALUES
    ...
    ) AS v(name, country, rate, description, is_default, is_active)
    WHERE NOT EXISTS (
      SELECT 1 FROM vat_rates existing
      WHERE existing.name = v.name AND existing.country = v.country
    )
  `;
