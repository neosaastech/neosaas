...
Releases on `release-live` are fully automated by [semantic-release](https://github.com/semantic-release/semantic-release) (`.github/workflows/release-live.yml`, config in `.releaserc.json`) — no manual version bump, no release PR to merge.

1. Push a [Conventional Commit](https://www.conventionalcommits.org/) (`fix:`, `feat:`, `feat!:`/`BREAKING CHANGE:`) to `release-live`.
2. semantic-release analyzes commits since the last tag to compute the next patch/minor/major version, updates `CHANGELOG.md` and `package.json`, and publishes a GitHub Release — immediately, on that same push.
3. `scripts/release-sync-boilerplate.sh` (invoked by semantic-release's publish step) syncs the filtered boilerplate to the public `neosaastech/neosaas` repo, tags + releases it there too, and notifies `neosaas-website` — this is the mechanism client sites' `apply-update.yml` pulls from.

`chore:`/`docs:`/`ci:`/`test:` commits never trigger a release on their own. See `CHANGELOG.md`'s Versioning Policy for the exact patch/minor/major rule.
