...
export async function GET(request: NextRequest) {
  try {
    ...
    const type = searchParams.get('type') // 'order' | 'payment' | 'user' | 'support' | 'system'
    ...
    for (const order of recentOrders) {
      ...
      // Interactive: physical products (need shipping)
      // Informative: digital products (auto-delivered)
      const orderMode = hasPhysical ? 'interactive' : 'informative'
      ...
      notifications.push({
        ...
        metadata: {
          ...
          hasPhysical,
          hasDigital,
          notificationType: hasDigital && !hasPhysical ? 'digital_info' : 'order',
    ...
    for (const msg of messagesQuery) {
      ...
      if (metadata.notificationType?.includes('order') || metadata.notificationType?.includes('digital')) {
