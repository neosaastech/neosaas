...
/**
 * Triggers the Kube-side rollout. **The receiver does not exist yet** — this
 * is the Next.js-side half only. UPDATE_ORCHESTRATOR_WEBHOOK_URL must point
 * to a real, separately-secured endpoint in the cluster with the narrow
 * permission to roll out this one deployment — building/hardening that
 * receiver is infra work, not something this route can responsibly invent.
 * Until it's configured, this returns 501 rather than silently no-op'ing.
 */
  ...
  if (!webhookUrl || !token) {
    return {
      ...
      body: {
        success: false,
        error:
          'Update orchestrator not configured (UPDATE_ORCHESTRATOR_WEBHOOK_URL / UPDATE_SYSTEM_TOKEN) — the Kube-side receiver for this action has not been built yet.',
      },
    }
  }
