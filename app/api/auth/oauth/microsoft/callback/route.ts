 ...
 *
 * Handles the Microsoft callback after authentication.
 */
...
export async function GET(request: NextRequest) {
  console.log("🔄 [Microsoft OAuth Callback] Callback received");
  ...
  try {
    ...
    if (error) {
      console.error(`❌ [Microsoft OAuth Callback] Microsoft error: ${error}`);
    ...
    // Validate the code
    if (!code) {
      console.error("❌ [Microsoft OAuth Callback] Missing code");
      return NextResponse.redirect(
    ...
    if (!savedState || savedState !== state) {
      console.error("❌ [Microsoft OAuth Callback] Invalid or missing state");
      return NextResponse.redirect(
        new URL("/auth/login?error=invalid_state", request.url)
      );
    }

    console.log(`✅ [Microsoft OAuth Callback] Code and state validated`);

    // Fetch configuration
    const config = await microsoftOAuthProvider.getConfiguration("production", request.url);
    if (!config) {
      console.error("❌ [Microsoft OAuth Callback] Configuration not found");
      return NextResponse.redirect(
    ...
    console.log("🔄 [Microsoft OAuth Callback] Exchanging code for access token...");
    ...
    console.log("✅ [Microsoft OAuth Callback] Access token obtained");

    // Fetch user information
    console.log("🔄 [Microsoft OAuth Callback] Fetching user info...");
    const microsoftUserInfo = await microsoftOAuthProvider.getUserInfo(accessToken);

    if (!microsoftUserInfo || !microsoftUserInfo.email) {
      console.error("❌ [Microsoft OAuth Callback] Could not retrieve email");
      return NextResponse.redirect(
        new URL("/auth/login?error=no_email", request.url)
      );
    }

    console.log(`✅ [Microsoft OAuth Callback] Microsoft user: ${microsoftUserInfo.name}`);
    ...
    console.log("🔄 [Microsoft OAuth Callback] Processing via OAuthUserService...");
    ...
    try {
      const result = await OAuthUserService.processOAuthUser({
        ...
        companyInfo: microsoftUserInfo.companyInfo, // Company data from Microsoft
        accessToken,
        refreshToken: tokenData.refreshToken,
      });

      if (!result) {
        console.error("❌ [Microsoft OAuth Callback] OAuth processing failed");
        return NextResponse.redirect(
          new URL("/auth/login?error=oauth_processing_failed", request.url)
        );
      }

      console.log(`✅ [Microsoft OAuth Callback] User processed`);
      ...
      console.error("❌ [Microsoft OAuth Callback] OAuth processing error:", oauthError);
