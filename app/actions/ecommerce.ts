...
import { db } from "@/db"
import { products, carts, cartItems, orders, orderItems, users, subscriptions } from "@/db/schema"
  ...
  try {
    ...
    for (const item of cart.items) {
      ...
      await db.insert(orderItems).values({
        orderId: order.id,
        itemType: item.product.type || "product", // Use actual product type (digital, physical, consulting)
