...
export function PricingGrid({ products }: { products: Product[] }) {
  ...
  const handlePurchase = async (product: Product) => {
    ...
    if (product.price === 0) {
      ...
      // If it's an appointment but no fileUrl (which we use as target URL), maybe redirect to a contact page or show a toast?
      // For now, let's assume fileUrl is the target.
      if (product.type === 'appointment') {
         // If we had a booking page, we would redirect there.
         // window.location.href = `/book/${product.outlookEventTypeId}`
         toast.info("Booking configuration missing. Please contact support.")
         return
      }
          ...
          >
            ...
            {isFree && (
              <div data-slot="card-footer" className="flex items-center px-6 [.border-t]:pt-6">
                ...
                >
                  {product.type === 'appointment' ? <Icons.Calendar className="mr-2 h-5 w-5" /> : <Download className="mr-2 h-5 w-5" />}
                  {product.type === 'appointment' ? 'Book Now' : 'Access Now'}
                </button>
              </div>
            )}
          </div>
