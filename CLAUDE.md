...
| `connector-system` | zoho-engine(8000), **github-engine**(8001 — RBAC multi-agent, Leon gestionnaire), vercel(8002), neon(8003), **penpot-engine**(8004), openprovider(8005), cloudflare(8006), stalwart(8007), google-discovery(8008), crawlee(8009), dataforseo(8010), **notion**(8011), **github-mcp**(8080 MCP streamable-http) |
| `rag-system` | Qdrant |
...
> **Camille v4.0 — Frontend Builder** (architecture deux phases Analyze→Execute, classification pages static/display/form/hybrid/fix, signal Guillaume pour backend, checkpoint Leon) : **[CLAUDE-camille.md](CLAUDE-camille.md)**
...
| **Leon** | Chef de Production — REVIEW (Notion+normes→spec) + TASK (CLARIFYING→dispatch) | FastAPI+Temporal | 8181 | `leon` | active v3.5 (REVIEW mode, notion_update_page, 9 intent labels dont `audit` + `design_deploy`, R9.13 classify cascade claude-sonnet→gpt-4o, zoho_delete_projects confirmed gate, R6, `delegate_to_charlotte` read-only audit, **`dispatch_design_and_backend` confirmed gate** — récapitulatif obligatoire avant dispatch, **github_grant_access/revoke** — gestionnaire RBAC github-engine, **GitHub tools** via github-connector: list/create/read/write/branch/PR, **Vercel tools** via vercel-connector: list/deploy/logs/create, **`design_deploy`**: `_call_joseph_tool` → gather spec+tokens → Camille apply_design_tokens → email) |
| **Camille** | Frontend Builder — GitHub repo (template-nextjs) + Vercel + Penpot export · **architecture v4.0 : [CLAUDE-camille.md](CLAUDE-camille.md)** | FastAPI | 8485 | — | active v3.3 (GitHub MCP + /mission, dispatché par Leon, `apply_design_tokens` + `generate_pages_from_penpot` + `camille_build_frontend` — **v4.0 en conception** : deux phases Analyze→Execute, classification pages, signal Guillaume) |
...
> **Leon v3.5 — Chef de Production** (mode REVIEW : notion_read_page + notion_update_page → spec corrigé → validation → Zoho ; mode TASK : Q0 Notion + CLARIFYING Charlotte pattern + dispatch déterministe design→Joseph ; `zoho_delete_projects` confirmed gate + protocole 3-étapes ; intent `audit` 3-axes normes/Zoho/doc + Patterns A/B/C + `cluster_status` Phase 3 INFRA + `delegate_to_charlotte` read-only services externes + MAD pre/post-store ; R9.13 cascade classify claude-sonnet→gpt-4o→mistral ; règle R6 connector ; **`dispatch_design_and_backend` confirmed gate** — récapitulatif + `confirmed=True` obligatoire ; **github_grant_access / github_revoke_access** — gestionnaire RBAC github-engine pour toutes créations/suppressions d'agents et fins de projets ; **GitHub tools** via github-connector Phase 1b : 8 outils list/create/read/write/branch/PR ; **Vercel tools** via vercel-connector Phase 1b : 6 outils list/deploy/logs/create/learn) : **[CLAUDE-leon.md](CLAUDE-leon.md)**
> **github-engine v2.0** : RBAC multi-agent (Leon gestionnaire), multi-org, audit Langfuse, remplacement github-connector : **[CLAUDE-github-engine.md](CLAUDE-github-engine.md)**
...
> Guide complet (classes A→E, interview 8 questions, Pattern A/B, 10 étapes auto, MAD, décommissionnement) : **[CLAUDE-create-agent.md](CLAUDE-create-agent.md)** · **[CLAUDE-agents.md](CLAUDE-agents.md)** · **[CLAUDE-agent-learning.md](CLAUDE-agent-learning.md)**
